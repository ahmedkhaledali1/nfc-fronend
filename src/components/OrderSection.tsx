...
const OrderSection = () => {
  ...
  const validateStep = (step: number): boolean => {
    switch (step) {
      case 1:
        return !!(orderData.fullName && orderData.phoneNumber && orderData.email && orderData.fullAddress);
      case 2:
            ...
            <div className="lg:col-span-1 fade-in">
              <Card className="card-premium sticky top-24">
                <CardHeader>
                  ...
                  <CardDescription>Professional networking made simple</CardDescription>
                </CardHeader>
                <CardContent className="space-y-6">
                  {/* Product Image */}
                  <div className="aspect-video rounded-lg overflow-hidden">
                    <img 
                      src={productImages[orderData.color as keyof typeof productImages]}
                  ...
                  <div>
                    ...
                    <RadioGroup 
                      ...
                      onValueChange={(value) => setOrderData({...orderData, color: value})}
                      className="flex space-x-4"
                    >
                      <div className="flex items-center space-x-2">
                        <RadioGroupItem value="black" id="black" />
                        <Label htmlFor="black" className="text-sm">Black Matte</Label>
                      </div>
                      <div className="flex items-center space-x-2">
                        <RadioGroupItem value="white" id="white" />
                        <Label htmlFor="white" className="text-sm">White Glossy</Label>
                      </div>
                    </RadioGroup>
                  </div>
              ...
              <Card className="card-premium">
                ...
                <CardContent className="space-y-6">
                    ...
                    <div className="space-y-4">
                      ...
                      <div>
                        ...
                        <Input
                          ...
                          onChange={(e) => setOrderData({...orderData, linkedinUrl: e.target.value})}
                          ...
                          <Label htmlFor="cash" className="flex-1 cursor-pointer">
                            <div className="flex items-center justify-between">
                              <div>
                                ...
                                <div className="text-sm text-muted-foreground">Pay when you receive your card</div>
                              </div>
                  ...
                  {currentStep === 3 && (
                    <div className="space-y-6">
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div>
                          ...
                          <div className="space-y-2 text-sm">
                            <div><span className="text-muted-foreground">Name:</span> {orderData.fullName}</div>
                            <div><span className="text-muted-foreground">Phone:</span> {orderData.phoneNumber}</div>
                            <div><span className="text-muted-foreground">Email:</span> {orderData.email}</div>
                            {orderData.linkedinUrl && (
                              <div><span className="text-muted-foreground">LinkedIn:</span> {orderData.linkedinUrl}</div>
                            )}
                          </div>
                        </div>
                        ...
                        <div>
                          ...
                          <div className="space-y-2 text-sm">
                            <div><span className="text-muted-foreground">Product:</span> Premium NFC Business Card</div>
                            <div><span className="text-muted-foreground">Color:</span> {orderData.color === 'black' ? 'Black Matte' : 'White Glossy'}</div>
                            <div><span className="text-muted-foreground">Payment:</span> Cash on Delivery</div>
                            <div><span className="text-muted-foreground">Total:</span> <span className="font-semibold">35 JOD</span></div>
                          </div>
                        </div>
                      </div>
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">{orderData.fullAddress}</p>
                      </div>
                    </div>
                  )}
                  ...
                  <div className="flex justify-between pt-6 border-t border-border">
                    ...
                    {currentStep < 3 ? (
                      <Button onClick={nextStep} className="btn-hero flex items-center">
                        Next
